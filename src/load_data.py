...
MILVUS_PORT = "19530"
COLLECTION_NAME = "diario_uniao"
CSV_FILE_PATH = "C:/projetos-ia/milvus/data/diario_uniao.csv"

# --- Modelo de Embedding ---
# Usando um modelo pré-treinado da biblioteca sentence-transformers.
# 'all-MiniLM-L6-v2' é um bom ponto de partida, produzindo vetores de 384 dimensões.
...
def create_collection_if_not_exists():
    ...
    # Define o esquema (schema) para a coleção
    fields = [
        ...
        FieldSchema(name="resumo_texto", dtype=DataType.VARCHAR, max_length=65535), # Armazena o texto original
        FieldSchema(name="embedding", dtype=DataType.FLOAT_VECTOR, dim=VECTOR_DIMENSION)
    ]
    schema = CollectionSchema(fields, description="Coleção para documentos do Diário Oficial da União")
    
    collection = Collection(name=COLLECTION_NAME, schema=schema)
    print(f"Coleção '{COLLECTION_NAME}' criada com sucesso.")
    
    # Cria um índice para o campo vetorial
...
def load_csv_and_prepare_data():
    ...
    orgaos = [row['orgao'] for row in data]
    tipos_ato = [row['tipo_ato'] for row in data]
    datas_publicacao = [row['data_publicacao'] for row in data]
    secoes = [row['secao'] for row in data]
    resumos = [row['resumo_texto'] for row in data]

    print("Gerando embeddings para o texto...")
    embeddings = embedding_model.encode(resumos)
    print("Embeddings gerados.")

    # Prepara as entidades para inserção no Milvus
    entities = [
        ids,
        orgaos,
        tipos_ato,
...
def main():
    ...
    try:
        ...
        # Verifica se a coleção está vazia antes de inserir
        if collection.num_entities > 0:
            ...
            collection.flush() # Realiza o flush para tornar os dados pesquisáveis
            print(f"{len(entities_to_insert[0])} entidades inseridas com sucesso em '{COLLECTION_NAME}'.")

        # Carrega a coleção em memória para pesquisa
        print("Carregando coleção em memória...")
        collection.load()
